...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    //std::cout << "rank - " << rank << " jbeg - " << jbeg << " jend - " << jend << std::endl;//работает
    while (1) 
    {
        //std::cout <<"\n" << "rank - " << rank << "\n";

        for (i = jbeg; i <= jend; i++) //возможно будет проблема с послежним элементом
        {
        ...
        maxdelta = delta;
        //std::cout << "rank - "<<rank  << " delta - " << delta << "\n";
        ...
        if (rank == 0)
        {
            ...
            //std::cout << "rank - " << rank << "recive - " << unew[jend + 1] << " from " << 1 << "\n";
        }
        else if (rank != size - 1)
        {
           ...
           // std::cout << "rank - " << rank << "recive - " << unew[jend + 1] << " from " << rank + 1 << "\n";
            if (MPI_Recv(&unew[jbeg - 1], 1, MPI_DOUBLE, rank - 1, 2, MPI_COMM_WORLD, NULL) != MPI_SUCCESS)
            {
                MPI_Abort(MPI_COMM_WORLD, 1);
            }
            //std::cout << "rank - " << rank << "recive - " << unew[jbeg - 1] << " from " << rank - 1 << "\n";
